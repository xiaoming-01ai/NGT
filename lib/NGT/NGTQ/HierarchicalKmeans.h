  ...
  public:
	...
	for (auto &member : cluster.members) {
	  if (member.vectorID > leafNode.members.size()) {
	    CERR <<  " "  << "Fatal error. member:" << member.vectorID << ":" << leafNode.members.size() << std::endl;
	    abort();
	  }
	  if (member.vectorID == leafNode.members.size()) {
    ...
    static double computeError(std::vector<HKNode*> &nodes, NGT::ObjectSpace &objectSpace, QBGObjectList &objectList) {
      CERR <<  " "  << "node size=" << nodes.size() << std::endl;
      double distance = 0.0;
      size_t dcount = 0;
      for (auto *node : nodes) {
	if (node->leaf) {
	  ...
	  for (auto &child : internalNode.children) {
	    if (nodes[child.first]->leaf) {
	      if (dcount % 100000 == 0) {
		CERR <<  " "  << "Processed leaves=" << dcount << std::endl;
	      }
	...
	if (node->leaf) {
	  ...
	  for (auto &child : internalNode.children) {
		...
		CERR <<  " "  << "leaf ID is invalid?" << std::endl;
	      }
	      leafID++;
	      size_t count = 0;
	      clusterCount++;
      ...
      CERR <<  " "  << "# of clusters=" << clusterCount << std::endl;
      return objectCount;
    }
    ...
    static size_t extractIndex(std::ostream &oStream, std::vector<HKNode*> &nodes, size_t numOfObjects) {
      ...
      CERR <<  " "  << "numOfObjects=" << numOfObjects << std::endl;
      CERR <<  " "  << "node size=" << nodes.size() << std::endl;
      for (auto *node : nodes) {
	if (node->leaf) {
	  HKLeafNode &leafNode = static_cast<HKLeafNode&>(*node);
	  for (auto &member : leafNode.members) {
	    if (member > numOfObjects) {
	      CERR <<  " "  << "output index: Internal fatal error. " << member << ":" << numOfObjects - 1 << std::endl;
	      abort();
	    }
	    if (member == 0) {
	      CERR <<  " "  << "output index: Internal fatal error. Invalid ID" << std::endl;
	      abort();
	    }
      ...
      CERR <<  " "  << "clusterID.size=" << clusterID.size() << std::endl;
      size_t count = 0;
      for (auto cid : clusterID) {
	count++;
	oStream << cid << std::endl;
      }
      CERR <<  " "  << "# of id=" << count << std::endl;
      return count;
    }
    ...
    static void extractBtoQAndQCentroid(std::ostream &btoqStream, std::ostream &qStream,
      ...
      CERR <<  " "  << "extractBtoQ" << std::endl;
      std::vector<int32_t> btoq(numOfThirdClusters);
      CERR <<  " "  << "numOfThirdClusters=" << numOfThirdClusters << std::endl;
      CERR <<  " "  << "node size=" << nodes.size() << std::endl;
      size_t rootID = 0;
      HKInternalNode &root = static_cast<HKInternalNode&>(*nodes[rootID]);
      CERR <<  " "  << "first=" << root.children.size() << std::endl;
	...
	for (auto &c2 : node1.children) {
	  ...
	  for (auto &c3 : node2.children) {
	    ...
	    if (leaf.id != leafID++) {
	      CERR <<  " "  << "leaf is invalid" << leaf.id << ":" << leafID << std::endl;
	      abort();
	    }
	  }
	  qID++;
      ...
      CERR <<  " "  << "second=" << secondCount << std::endl;
      CERR <<  " "  << "third=" << thirdCount << std::endl;
      CERR <<  " "  << "object=" << objectCount << std::endl;
    }
	...
	if (node->leaf) {
	  ...
	  if (numOfRandomObjects >= leafNode.members.size()) {
		...
		CERR <<  " "  << "Internal error. " << idx << ":" << leafNode.members.size() << std::endl;
		abort();
	      }
		...
		if (idx >= leafNode.members.size()) {
	      ...
	      CERR <<  " "  << "IDX=" << idx << "/" << leafNode.members.size() << std::endl;
	      randomObjectIDXs.push_back(leafNode.members[idx]);
	    }
	  }
	  CERR <<  " "  << "randomObjectIDXs=" << randomObjectIDXs.size() << std::endl;
	  for (auto member : randomObjectIDXs) {
	    if (member == 0) {
	      CERR <<  " "  << "output index: Internal fatal error. Invalid ID. " <<  member << std::endl;
	      abort();
	    }
	    std::vector<float> object;
	    quantizer.objectList.get(member, object, &quantizer.globalCodebookIndex.getObjectSpace());
	    if (leafNode.id >= numOfObjects) {
	      CERR <<  " "  << "Internal error! Wrong leaf ID. " << leafNode.id << ":" << numOfObjects << std::endl;
	      abort();
	    }
	...
	if (extractCentroid && c.empty()) {
	  CERR <<  " "  << "qbg: Fatal error! The centroid is empty." << std::endl;
	  abort();
	}
	...
	for (auto &ro : ros) {
	  if (ro.empty()) {
	    CERR <<  " "  << "qbg: Fatal error! The random object vector is empty." << std::endl;
	    abort();
	  }
    ...
    static void hierarchicalKmeans(uint32_t id, int32_t rootID, std::vector<float> &object,
		...
		if (nodeID < 0)
		{
			CERR <<  " "  << "Fatal inner error! node ID=" << nodeID << std::endl;
			exit(1);
		}
		auto *node = nodes[nodeID];
		HKLeafNode &leafNode = static_cast<HKLeafNode &>(*node);
		if (leafNode.members.size() >= maxSize)
		{
			...
			subtimer.stop();
			CERR <<  " "  << "aggregate time=" << subtimer << std::endl;
			subtimer.start();
			split(id, vectors, nodes, nodeID, clustering);
			subtimer.stop();
			CERR <<  " "  << "split time=" << subtimer << std::endl;
		}
		else
		{
...
#pragma omp critical
		...
		for (size_t idx = 0; idx < batch.size(); idx++)
		{
			...
			if (leafNode.members.size() > maxSize)
			{
				...
				{
					if (static_cast<int32_t>((*i).second) == nodeIDs[idx])
						break;
				}
...
#pragma omp critical
		...
		CERR <<  " "  << "exceeded leaves=" << exceededLeaves.size() << std::endl;
		for (size_t idx = 0; idx < exceededLeaves.size(); idx++)
		{
	...
	{
		CERR <<  " "  << "numOfTotalClusters=" << numOfTotalClusters << std::endl;
		CERR <<  " "  << "numOfLeaves=" << numOfLeaves << std::endl;
		if (numOfLeaves > numOfTotalClusters)
		{
			CERR <<  " "  << "# of clusters is invalid. " << numOfLeaves << ":" << numOfTotalClusters << std::endl;
			abort();
		}
		...
		for (size_t nidx = 0; nidx < nodeSize; nidx++)
		{
			if (nodes[nidx]->leaf)
			{
				...
				{
					CERR <<  " "  << "Processed leaves: " << leafCount << " " << leafCount * 100 / numOfLeaves << "%" << std::endl;
				}
				...
				if (nodes[nidx]->leaf)
				{
					CERR <<  " "  << "At this moment, the second node should be an internal" << std::endl;
					abort();
				}
			}
		}
																 ...
																 std::vector<HKNode *> &nodes, NGT::Clustering::InitializationMode initMode)
	{
		...
		for (size_t nidx = 0; nidx < nodes.size(); nidx++)
		{
			if (nodes[nidx]->leaf)
			{
				leafCount++;
				{
					...
					{
						CERR <<  " "  << "Processed leaves: " << leafCount << " " << leafCount * step / numOfLeaves << "%" << std::endl;
					}
				}
				HKLeafNode &leafNode = static_cast<HKLeafNode &>(*nodes[nidx]);
				size_t nClusters = round(static_cast<float>(leafNode.members.size()) / numOfRemainingVectors * numOfRemainingClusters);
		...
		timer.stop();
		CERR <<  " "  << "hierarchicalKmeansWithNumberOfClustersInParallel: extract leaves. Time=" << timer << std::endl;
		timer.start();

		CERR <<  " "  << "start kmeans..." << std::endl;
...
#pragma omp critical
		...
		timer.stop();
		CERR <<  " "  << "hierarchicalKmeansWithNumberOfClustersInParallel: kmeans. Time=" << timer << std::endl;
		timer.start();

		CERR <<  " "  << "add nodes..." << std::endl;
		...
		timer.stop();
		CERR <<  " "  << "hierarchicalKmeansWithNumberOfClustersInParallel: add nodes. Time=" << timer << std::endl;
	}
					   ...
					   size_t maximumIteration = 1000)
	{
		...
		CERR <<  " "  << "numOfRemainingClusters=" << numOfRemainingClusters << std::endl;
		CERR <<  " "  << "numOfRemainingVectors=" << numOfRemainingVectors << std::endl;
		CERR <<  " "  << "upperClusters=" << upperClusters.size() << std::endl;
		CERR <<  " "  << "total=" << ts << ":" << numOfLowerClusters << std::endl;
		if (ts < numOfLowerClusters || numOfRemainingClusters != 0)
		{
			CERR <<  " "  << "subclustering: Internal error! " << std::endl;
			exit(1);
		}
...
#pragma omp critical
			...
			{
				CERR <<  " "  << "the sizes of members are not consistent" << std::endl;
				abort();
			}
			...
			{
				CERR <<  " "  << "the sizes of cluster members are not consistent" << std::endl;
				abort();
			}
		}
		size_t nc = 0;
		...
		CERR <<  " "  << "# of clusters=" << nc << " # of members=" << mc << std::endl;
	}
	void subclustering(std::vector<NGT::Clustering::Cluster> &upperClusters, size_t numOfLowerClusters, size_t numOfObjects,
					   NGT::ObjectSpace &objectSpace, QBGObjectList &objectList,
					   NGT::Clustering::InitializationMode initMode,
							  ...
							  size_t maximumIteration = 1000)
	{
		...
		CERR <<  " "  << "upperClusters=" << upperClusters.size() << std::endl;
		CERR <<  " "  << "total=" << ts << ":" << numOfLowerClusters << std::endl;
		CERR <<  " "  << "max iteration=" << maximumIteration << std::endl;
		timer.stop();
...
#pragma omp parallel for schedule(dynamic)
		for (size_t idx = 0; idx < upperClusters.size(); idx++)
		{
			...
			if (nPartialClusters[idx] != lowerClusters[idx].size())
			{
				CERR <<  " "  << "Warning: the sizes of cluster members are inconsistent. " << nPartialClusters[idx] << ":" << lowerClusters[idx].size() << ":" << idx << std::endl;
			}
			counters[threadid]++;
			{
				size_t cnt = 0;
				...
				if (cnt % progressStep == 0)
				{
					...
					CERR <<  " "  << "subclustering: " << cnt << " clusters ("
		...
		for (size_t idx = 0; idx < upperClusters.size(); idx++)
		{
			...
			auto threadid = omp_get_thread_num();
			{
				for (auto &m : upperClusters[idx].members)
				{
					if (threadid >= nthreads)
					{
						CERR <<  " "  << "inner fatal error. # of threads=" << nthreads << ":" << threadid << std::endl;
						exit(1);
					}
...
#pragma omp parallel for
		for (size_t id = beginID; id <= endID; id++)
		{
			...
			if (minidx < 0)
			{
				CERR <<  " "  << "assign: Fatal error!" << std::endl;
				abort();
			}
#pragma omp critical
			{
				...
				if (count % 1000000 == 0)
				{
					CERR <<  " "  << "# of assigned objects=" << count << std::endl;
				}
			}
		}
	}
					   ...
					   NGT::ObjectSpace &objectSpace, QBGObjectList &objectList)
	{
		...
		{
			CERR <<  " "  << "Cannot open multiple streams." << std::endl;
			abort();
		}
#endif
...
#pragma omp critical
	...
	{
		CERR <<  " "  << "optimizeEpsilon: expectedRecall=" << expectedRecall << std::endl;
...
#pragma omp parallel for
			...
			CERR <<  " "  << "Info: # of not exact results=" << notExactResultCount << " start epsilon=" << startEpsilon
							  ...
							  float expectedRecall = 0.98)
	{
		if (beginID > endID)
		{
			CERR <<  " "  << "assignWithNGT::Warning. beginID:" << beginID << " > endID:" << endID << std::endl;
			return;
		}
		...
		for (size_t cidx = 0; cidx < clusters.size(); cidx++)
		{
			if (cidx % 100000 == 0)
			{
				CERR <<  " "  << "# of appended cluster objects=" << cidx << std::endl;
			}
			anngIndex.append(clusters[cidx].centroid);
		}
		CERR <<  " "  << "createIndex..." << std::endl;
		...
		{
			CERR <<  " "  << "Warning. remove is failed. " << com << std::endl;
		}
#else
		NGT::Index &index = anngIndex;
#endif
		CERR <<  " "  << "assign with NGT..." << std::endl;
		endID++;
#ifdef MULTIPLE_OBJECT_LISTS
		if (!objectList.openMultipleStreams(omp_get_max_threads()))
		{
		...
		timer.stop();
		CERR <<  " "  << "assignWithNGT: exploring epsilon. time=" << timer << " epsilon=" << epsilon << std::endl;
		timer.start();
...
#pragma omp parallel for
		for (size_t id = beginID; id < endID; id++)
		{
			...
			distances[threadID].second += objects[0].distance;
			{
				...
				if (cnt % progressStep == 0)
				{
					...
					CERR <<  " "  << "assignWithNGT: " << cnt << " objects ("
							  << progress << "%) have been assigned. time=" << timer << std::endl;
					timer.restart();
				}
			}
		}
		CERR <<  " "  << "pushing..." << std::endl;
		for (size_t id = beginID; id < endID; id++)
		{
		...
		}
		{
			...
			if (n != 0)
			{
				...
				CERR <<  " "  << "assign. quantization error=" << err << "/" << n << std::endl;
			}
			else
			{
				CERR <<  " "  << "assign. no assigned vectors." << std::endl;
			}
		}
	}
	...
	{
		CERR <<  " "  << "clustering with two layers... # of clusters=" << numOfClusters << std::endl;
		if (numOfClusters == 0)
		{
		...
		numOfFirstObjects = numOfObjectsForEachFirstCluster == 0 ? numOfFirstObjects : numOfFirstClusters * numOfObjectsForEachFirstCluster;
		size_t numOfSecondClusters = numOfClusters;
		size_t numOfSecondObjects = vectors.size();
		numOfSecondObjects = numOfObjectsForEachSecondCluster == 0 ? numOfSecondObjects : numOfSecondClusters * numOfObjectsForEachSecondCluster;
		...
		NGT::Timer timer;
		{
			...
			CERR <<  " "  << "clustering for the first...." << std::endl;
			timer.start();
			firstClustering.kmeans(firstVectors, numOfFirstClusters, firstClusters);
			timer.stop();
			CERR <<  " "  << "clustering for the first. # of clusters=" << firstClusters.size() << " time=" << timer << std::endl;
		}
		...
		timer.stop();
		CERR <<  " "  << "assign for the second. time=" << timer << std::endl;

		CERR <<  " "  << "subclustering for the second..." << std::endl;
		...
		timer.stop();
		CERR <<  " "  << "subclustering for the second. time=" << timer << std::endl;
	}
