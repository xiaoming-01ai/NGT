...
namespace QBG {
  ...
  public:
      ...
      generateResidualObjects(vector<vector<float>> &globalCentroid, vector<vector<float>> &vectors)
    {
      ...
      CERR <<  " "  << "generateResidualObjects: Not implemented." << std::endl;
      abort();
#else
	...
	CERR <<  " "  << "optimizer: Warning. The dimension is inconsistency. " << globalCentroid[0].size() << ":" << vectors[0].size() << std::endl;
      }
	...
	index.search(query);
	if (gc.empty()) {
	  CERR <<  " "  << "inner fatal error. no results! something wrong." << std::endl;
	  abort();
	}
	if (gc[0].id == 0 || gc[0].id > globalCentroid.size()) {
	  CERR <<  " "  << "wrong id " << gc[0].id << ":" <<  globalCentroid.size() << std::endl;
	  abort();
	}
	...
	try {
	  ...
	  CERR <<  " "  << err.what() << ":" << v.size() << "x" << globalCentroid[gcidx].size() << std::endl;
	  abort();
	}
      }
#endif
    ...
    static void optimizeRotation(
	  	...
	  	CERR  << "clusterIteration " << clusterIteration <<  endl; 
	  	CERR  << "numberOfSubvectors " <<  numberOfSubvectors <<  endl; 
	  	CERR  << "subvectorSize " << subvectorSize <<  endl; 
		CERR  << "iterator " << iteration << endl;
		CERR  << "vectors info " << vectors.size() << " . per size " << vectors[0].size() << endl;
...
#pragma omp parallel for
			for (size_t m = 0; m < numberOfSubvectors; m++)
			{
				...
				CERR << "subVectors " << subVectors.size() << " dim " << subVectors[0].size() << endl;
			...
			{
				CERR <<  " "
						  << "Optimizer: Warning. The elapsed time exceeded the limit-time. " << timelimit << ":" << timelimitTimer.time << std::endl;
	...
	{
		CERR <<  " "
				  << "Optimizer: vectors" << vectors.size() << " "
				  << "local clusters " << localClusters.size() << " "
				  << " rs size " << rs.size() << std::endl;
		if (vectors.size() == 0)
		{
