...
namespace NGTQ {
  ...
  public:
    Rotation &operator=(const std::vector<float> &r)
    {
      ...
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Rotation: Fatal inner error! Invalid data. " << dim * dim << ":" << PARENT::size() << std::endl;
        abort();
      }
      return *this;
    }
    ...
    void mul(std::vector<float> &a)
    {
      if (a.size() != dim)
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Fatal inner error! " << a.size() << ":" << dim << std::endl;
        abort();
      }
      mul(a.data());
    }
    ...
    void deserialize(std::ifstream &is)
    {
      ...
      if (dim * dim != v)
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "rotation::deserialize: Fatal inner error. Invalid data. " << dim << ":" << dim * dim << ":" << v << std::endl;
        abort();
      }
    ...
    void show()
    {
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "R(" << dim << ")=" << std::endl;
      for (size_t i = 0; i < dim; i++)
      {
        for (size_t j = 0; j < dim; j++)
        {
          std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << (*this)[i * dim + j] << " ";
        }
  ...
  public:
    ...
    void buildIndex()
    {
      ...
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "QuantizationCodebook::buildIndex" << std::endl;
      if (index != 0)
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Quantization codebook: something wrong?" << std::endl;
        delete index;
      }
      ...
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "QuantizationCodebook::buildIndex # of the centroids=" << noOfCentroids << std::endl;
      for (size_t idx = 0; idx < noOfCentroids; idx++)
      {
        if ((idx + 1) % 100000 == 0)
        {
          std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "QuantizationCodebook::buildIndex processed objects=" << idx << std::endl;
        }
    ...
    size_t search(NGT::Object &object, float epsilon = 0.1)
    {
      ...
      if (result.size() == 0)
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "QuantizeationCodebook: Fatal Error! Cannot search." << std::endl;
        abort();
      }
      else
      {
  ...
  public:
    ...
    {
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "InvertedIndexObject: operator=() should not be used, because it cannot be implemented. Instead of this, use copy()." << std::endl;
      abort();
    }
...
#pragma omp parallel for
  ...
  public:
    ...
    public:
      ...
      {
#ifdef NGTQ_QBG
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "isValid() is not implemented" << std::endl;
        abort();
#else
        return flag[idx];
#endif
    ...
    inline void createFloatL2DistanceLookup(void *object, size_t sizeOfObject, void *globalCentroid, DistanceLookupTableUint8 &distanceLUT)
    {
      ...
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Individual scale offset compression is not implemented." << std::endl;
#endif
      distanceLUT.totalOffset = offset * static_cast<float>(localCodebookNo);
      auto *blutptr = distanceLUT.localDistanceLookup;
      flutptr = &flut[0];
             ...
             size_t sizeoftype, size_t dim, Rotation *r)
    {
      ...
      if (globalCodebookIndex->getObjectSpace().getRepository().size() == 2)
      {
        ...
        {
          std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Cannot load the global centroid. id=" << id << std::endl;
        }
      }
  ...
  public:
    ...
    {
      cerr << __FILE__ << "::" << __LINE__ << " " <<"operator() is not implemented" << endl;
      abort();
      return 0.0;
    }
  ...
  inline void operator()(void *inv, float *distances, size_t noOfObjects, DistanceLookupTableUint8 &distanceLUT)
  {
    ...
    while (localID < last)
    {
...
#ifdef NGTQBG_MIN
      {
        ...
        // std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "tmpmin=" << tmpmin << std::endl;
        if (min > tmpmin)
          min = tmpmin;
      }
#endif
...
#error "AVX512 is *NOT* defined. *INDIVIDUAL* scale offset compression is available only for AVX512!"
  ...
  inline void operator()(void *inv, float *distances, size_t noOfObjects, DistanceLookupTableUint8 &distanceLUT)
  {
    ...
    while (localID < last)
    {
      ...
      while (localID < lastgroup256)
      {
        __m256i lookupTable = _mm256_loadu_si256((__m256i const *)lut);
        _mm_prefetch(&localID[0] + 64 * 8, _MM_HINT_T0);
        // std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "obj=" << (int)(localID[0] & 0x0f) << "," << (int)((localID[0] >> 4) & 0x0f) << std::endl;
        __m256i packedobj = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i const *)&localID[0]));
        __m256i lo = _mm256_and_si256(packedobj, mask256x0F);
        __m256i hi = _mm256_slli_epi16(_mm256_and_si256(packedobj, mask256xF0), 4);
        __m256i obj = _mm256_or_si256(lo, hi);
        // std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "LUT=" << (int)*lut << "," << (int)*(lut+1) << std::endl;
        __m256i vtmp = _mm256_shuffle_epi8(lookupTable, obj);
  ...
  virtual void insert(vector<float> &object, vector<pair<NGT::Object *, size_t>> &objects, size_t id) = 0;
    ...
    {
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "getApproximateDistance() is not implemented." << std::endl;
      abort();
    }
...
public:
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
    fprintf(stderr, "%s::%d create empty index\n", __FILE__, __LINE__);
  ...
  void open(const string &index, bool readOnly)
  {
    ...
    {
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "open: Warning. global codebook is empty." << std::endl;
    }
    size_t localCodebookNo = property.getLocalCodebookNo();
...
#else
    {
      ...
      if (!ifs)
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"Cannot open " << index + "/" + getInvertedIndexFile() << "." << endl;
        return;
      }
...
#ifdef NGTQBG_COARSE_BLOB
    {
      ...
      if (!ifs)
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << getGlobalToInvertedIndexFile() << " doesn't exist." << std::endl;
      }
      else
      {
    ...
    if (!objectList.open(index + "/obj", static_cast<ObjectFile::DataType>(property.dataType), property.distanceType, property.dimension))
    {
      ...
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Ignore. " << msg.str() << std::endl;
    }
#ifdef MULTIPLE_OBJECT_LISTS
    objectList.openMultipleStreams(omp_get_max_threads());
#endif
    ...
    if (globalProperty.objectType == NGT::Property::ObjectType::Float)
    {
      ...
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Invalid localIDByteSize : " << property.localIDByteSize << std::endl;
        abort();
      }
    ...
    else if (globalProperty.objectType == NGT::Property::ObjectType::Uint8)
    {
      ...
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Inconsistent localIDByteSize and ObjectType. " << property.localIDByteSize << ":" << globalProperty.objectType << std::endl;
        abort();
      }
#ifdef NGTQ_VECTOR_OBJECT
      generateResidualObject = new GenerateResidualObjectFloat;
    ...
    {
      cerr << __FILE__ << "::" << __LINE__ << " " <<"NGTQ::open: Fatal Inner Error: invalid object type. " << globalProperty.objectType << endl;
      cerr << __FILE__ << "::" << __LINE__ << " " <<"   check NGT version consistency between the caller and the library." << endl;
      abort();
    }
...
#ifdef NGTQ_QBG
    {
      ...
      if (ifs)
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "loading the rotation..." << std::endl;
  ...
  void reconstructInvertedIndex(const string &invertedFile)
  {
    ...
    cerr << __FILE__ << "::" << __LINE__ << " " <<"Reconstruction is disabled!!!!!" << endl;
    return;
#endif
    cerr << __FILE__ << "::" << __LINE__ << " " <<"reconstructing to reduce shared memory..." << endl;
    ...
    for (size_t id = 0; id < size; ++id)
    {
      ...
      if (id % 100000 == 0)
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"Processed " << id << endl;
      }
    ...
    cerr << __FILE__ << "::" << __LINE__ << " " <<"verifying..." << endl;
    for (size_t id = 0; id < size; ++id)
    {
      if (invertedIndex.isEmpty(id))
      {
      ...
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<id << " : size is inconsistency" << endl;
      }
      for (size_t i = 0; i < esize; ++i)
      {
        ...
        {
          cerr << __FILE__ << "::" << __LINE__ << " " <<id << "," << i << " : id is inconsistency" << endl;
        }
        for (size_t d = 0; d < property.localDivisionNo; ++d)
        {
          if (sobject.localID[d] != dobject.localID[d])
          {
            cerr << __FILE__ << "::" << __LINE__ << " " <<id << "," << i << "," << d << " : local id is inconsistency" << endl;
          }
        }
      }
    }
  ...
  void createIndex(NGT::GraphAndTreeIndex &codebook,
                   ...
                   float &range)
  {
    fprintf(stderr, "%s::%d centroidLimit %lu\n", __FILE__, __LINE__, centroidLimit);
    if (centroidLimit > 0)
    {
  ...
  void setGlobalCodeToInvertedEntry(NGT::Index::InsertionResult &id, pair<NGT::Object *, size_t> &object, vector<LocalDatam> &localData)
  {
    ...
    else
    {
      if (property.centroidCreationMode != CentroidCreationModeDynamic)
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"Quantizer: Fatal error! Although it is a static global codebook, an object has been added to the global." << endl;
        cerr << __FILE__ << "::" << __LINE__ << " " <<"    The actual size of the global codebook=" << globalCodebookIndex.getObjectRepositorySize() - 1
             << ", The size of the global codebook in the property=" << property.globalCentroidLimit << std::endl;
        cerr << __FILE__ << "::" << __LINE__ << " " <<"    The both numbers above should be the same." << std::endl;
        cerr << __FILE__ << "::" << __LINE__ << " " <<"    Specify a proper size limitation for the global codebook?" << endl;
        assert(id.identical);
        abort();
      }
...
#pragma omp parallel for
  ...
  void constructLocalCodebooks()
  {
    ...
    {
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Fatal error. somethig wrong " << oft << ":" << globalCodebookIndex.getObjectSpace().getDimension() << std::endl;
      abort();
    }
  }
                                                     ...
                                                     float *subspaceObjects)
  {
    ...
    if (localCodebooks == 0)
    {
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Quantizer::setMultipleLocalCodeToInvertedEntry: FatalError!" << std::endl;
      abort();
    }
    {
#pragma omp parallel for
...
#pragma omp parallel for
  ...
  void buildMultipleLocalCodebooks(NGT::Index *localCodebook, size_t localCodebookNo, size_t numberOfCentroids)
  {
    ...
    for (size_t li = 0; li < localCodebookNo; ++li)
    {
      ...
      if (diff > 0.0)
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"Not converge. " << diff << endl;
      }
      cerr << __FILE__ << "::" << __LINE__ << " " <<"Clustering of the subvector is complete. " << localCodebook[li].getPath() << ":" << localCodebook[li].getObjectRepositorySize() << endl;
    }
  }
  ...
  void replaceInvertedIndexEntry(size_t localCodebookNo)
  {
    ...
    for (size_t gidx = 1; gidx < invertedIndex.size(); gidx++)
    {
      ...
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "replaceInvertedIndexEntry: Warning. empty inverted index entry. " << gidx << ":" << invertedIndex.size() << std::endl;
        continue;
      }
  ...
  {
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "insert() is not implemented." << std::endl;
    abort();
  }
#endif
                   ...
                   float &range, NGT::Index *gqindex)
  {
    ...
    {
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Fatal error. quantizationCodebook is empty" << std::endl;
      abort();
    }
    ids.clear();
...
#pragma omp critical
...
#endif
  {
    ...
    for (size_t idx = 0; idx < objects.size(); idx++)
    {
...
#ifdef GET_BLOB_EVAL
      {
        ...
        sc.setEpsilon(0.1);
        globalCodebookIndex.search(sc);
        // std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "insert:Eval: ";
...
#ifdef GET_BLOB_EVAL
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << identicalObjectCount << "/" << objects.size() << std::endl;
#endif
    return;
  }
  ...
  {
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "buildGlobalCodebookWithQIDIndex: Not implemented." << std::endl;
    abort();
#else
      NGT::Property property;
      ...
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "warning: the inverted index size is too small. Cannot build a global codebook with qid. " << invertedIndex.size() << ":" << globalCodebookIndex.getObjectRepositorySize() << std::endl;
        return index;
      }
      for (size_t id = 1; id < globalCodebookIndex.getObjectRepositorySize(); id++)
      {
        ...
        if (id % 100000 == 0)
        {
          std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "# of processed objects=" << id << ", vm size="
      ...
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "creating the index..." << std::endl;
      index->createIndex(50);
      return index;
#endif
  }
...
#pragma omp parallel for
  ...
  {
#ifdef NGTQ_SHARED_INVERTED_INDEX
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "enode: Not implemented." << std::endl;
    abort();
#else
...
#endif
#ifdef NGTQ_SHARED_INVERTED_INDEX
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "insert: Not implemented." << std::endl;
    abort();
#else
...
#pragma omp parallel for
      ...
      if (property.singleLocalCodebook)
      {
        ...
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "insert: Fatal Error. single local codebook isn't available." << std::endl;
        abort();
      }
      else
      {
...
#pragma omp parallel for
  ...
  {
    fprintf(stderr, "%s::%d create index beginID=%lu endID=%lu\n", __FILE__, __LINE__, beginID, endID);
    if (beginID == 0) {
      return;
    }
    NGT::Index *gqindex = 0;
    ...
    for (size_t id = beginID; id <= endID; id++)
    {
      ...
      {
        timer.stop();
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "# of processed objects=" << id << ", time=" << timer << ", vm size="
                  << NGT::Common::getProcessVmSizeStr() << "/"
                  << NGT::Common::getProcessVmPeakStr() << std::endl;
        timer.restart();
      }
      ...
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Cannot get object. ID=" << id << std::endl;
        continue;
      }
#else
  ...
  {
#if !defined(NGTQ_QBG)
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "setupInvertedIndex: Not implemented." << std::endl;
    abort();
#else
    if (globalCodebookIndex.getObjectRepositorySize() != codebookIndex.size() + 1)
    {
      std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "Warning: Error? " << globalCodebookIndex.getObjectRepositorySize() << ":" << codebookIndex.size() + 1 << std::endl;
    }
    ...
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "codebook index size=" << codebookIndex.size() << std::endl;
  ...
  void validate()
  {
    ...
    for (size_t gidx = 1; gidx < 4 && gidx < gcbSize; gidx++)
    {
      ...
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"something wrong" << endl;
        exit(1);
      }
      ...
      globalCodebookIndex.getObjectSpace().getRepository().extractObject(&gcentroid, gco);
      {
        ...
        {
          cerr << __FILE__ << "::" << __LINE__ << " " <<"a global centroid id is wrong in the inverted index." << gidx << ":" << invertedIndexEntry.id << endl;
          exit(1);
        }
      }
      ...
      if (gco != gcolist)
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"Fatal error! centroid in NGT is different from object list in NGTQ" << endl;
        exit(1);
      }
      ...
      for (size_t iidx = 0; iidx < (*invertedIndex[gidx]).size(); iidx++)
      {
        ...
        cerr << __FILE__ << "::" << __LINE__ << " " <<"  object ID=" << invertedIndexEntry.id;
        {
          ...
          cerr << __FILE__ << "::" << __LINE__ << " " <<":distance=" << distance;
        }
        cerr << __FILE__ << "::" << __LINE__ << " " <<":local codebook IDs=";
        for (size_t li = 0; li < property.localDivisionNo; li++)
        {
          cerr << __FILE__ << "::" << __LINE__ << " " <<invertedIndexEntry.localID[li] << " ";
        }
        ...
        for (size_t li = 0; li < property.localDivisionNo; li++)
        {
          if (invertedIndexEntry.localID[li] == 0)
          {
            ...
            cerr << __FILE__ << "::" << __LINE__ << " " <<"local ID is unexpected zero." << endl;
          }
        }
      }
      vector<size_t> ngid;
      {
        ...
        for (size_t resulti = 0; resulti < objects.size(); resulti++)
        {
          ...
          else
          {
            ...
            {
              cerr << __FILE__ << "::" << __LINE__ << " " <<" Strange! ";
              cerr << __FILE__ << "::" << __LINE__ << " " <<result[0].distance << ":" << objects[resulti].distance << " ";
            }
          }
          cerr << __FILE__ << "::" << __LINE__ << " " <<"  search object " << resulti << " ID=" << objects[resulti].id << " distance=" << objects[resulti].distance << endl;
        }
      }
  ...
  {
#ifdef NGTQ_TRACE
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "searchGlobalCodebook codebookSearchSize=" << codebookSearchSize << std::endl;
#endif
  ...
  {
#ifdef NGTQ_SHARED_INVERTED_INDEX
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "InvertedIndex: Not implemented." << std::endl;
    abort();
#else
    ...
    for (size_t idx = 0; idx < invertedIndex[gid]->size(); idx++)
    {
      ...
      {
        std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "you should change the object ID type." << std::endl;
        abort();
      }
#else
  ...
  inline void aggregateObjects(NGT::Object *query, size_t size, NGT::ObjectDistances &objects, NGT::ObjectSpace::ResultSet &results, size_t approximateSearchSize, AggregateObjectsFunction aggregateObjectsFunction)
  {
    for (size_t i = 0; i < objects.size(); i++)
    {
      if (invertedIndex[objects[i].id] == 0)
      {
        if (property.centroidCreationMode == CentroidCreationModeDynamic)
        {
          cerr << __FILE__ << "::" << __LINE__ << " " <<"Inverted index is empty. " << objects[i].id << endl;
        }
        continue;
      }
              ...
              double epsilon = FLT_MAX)
  {
    ...
    case AggregationModeApproximateDistance:
      ...
      break;
    default:
      cerr << __FILE__ << "::" << __LINE__ << " " <<"NGTQ::Fatal Error. invalid aggregation mode. " << aggregationMode << endl;
      abort();
    }
  ...
  {
#ifdef NGTQ_QBG
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << "calculateQuantizationError: Not implemented." << std::endl;
    return 0.0;
#else
    NGT::ObjectSpace &objectSpace = globalCodebookIndex.getObjectSpace();
    double distance = 0.0;
    ...
    std::cerr <<  __FILE__ << "::" << __LINE__ << " "  << distance << ":" << globalDistance << std::endl;
    return distance;
#endif
  }
  ...
  {
    cerr << __FILE__ << "::" << __LINE__ << " " <<"info" << endl;
  ...
  void verify()
  {
    cerr << __FILE__ << "::" << __LINE__ << " " <<"sizeof(LOCAL_ID_TYPE)=" << sizeof(LOCAL_ID_TYPE) << endl;
    size_t objcount = objectList.size();
    cerr << __FILE__ << "::" << __LINE__ << " " <<"Object count=" << objcount << endl;
    size_t gcount = globalCodebookIndex.getObjectRepositorySize();
    cerr << __FILE__ << "::" << __LINE__ << " " <<"Global codebook size=" << gcount << endl;
    size_t lcount = localCodebookIndexes[0].getObjectRepositorySize();
    cerr << __FILE__ << "::" << __LINE__ << " " <<"Local codebook size=" << lcount << endl;
    lcount *= 1.1;
    cerr << __FILE__ << "::" << __LINE__ << " " <<"Inverted index size=" << invertedIndex.size() << endl;

    cerr << __FILE__ << "::" << __LINE__ << " " <<"Started verifying global codebook..." << endl;
    vector<uint8_t> status;
    globalCodebookIndex.verify(status);

    cerr << __FILE__ << "::" << __LINE__ << " " <<"Started verifing the inverted index." << endl;
    size_t errorCount = 0;
    for (size_t i = 1; i < invertedIndex.size(); i++)
    {
      if (i % 1000000 == 0)
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"  verified " << i << " entries" << endl;
      }
      if (errorCount > 100)
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"Too many errors. Stop..." << endl;
        return;
      }
      if (invertedIndex[i] == 0)
      {
        cerr << __FILE__ << "::" << __LINE__ << " " <<"Warning inverted index is zero. " << i << endl;
        continue;
      }
      for (size_t j = 1; j < invertedIndex[i]->size(); j++)
      {
        ...
        {
          cerr << __FILE__ << "::" << __LINE__ << " " <<"The object ID of the inverted index entry is too big! " << invertedIndexEntry.id << ":" << objcount << endl;
          cerr << __FILE__ << "::" << __LINE__ << " " <<"  No. of the wrong entry in the inverted index is " << i << endl;
          errorCount++;
        }
        if (invertedIndexEntry.id == 0)
        {
          cerr << __FILE__ << "::" << __LINE__ << " " <<"The object ID of the inverted index entry is zero! " << invertedIndexEntry.id << ":" << objcount << endl;
          cerr << __FILE__ << "::" << __LINE__ << " " <<"  No. of the wrong entry in the inverted index is " << i << endl;
          errorCount++;
        }
        for (size_t li = 0; li < property.localDivisionNo; li++)
        {
          ...
          {
            cerr << __FILE__ << "::" << __LINE__ << " " <<"The local centroid ID of the inverted index entry is wrong. " << invertedIndexEntry.localID[li] << ":" << lcount << endl;
            cerr << __FILE__ << "::" << __LINE__ << " " <<"  No. of the wrong entry in the inverted index is " << i << ". No. of local centroid db is " << li << endl;
            errorCount++;
          }
          if (invertedIndexEntry.localID[li] == 0)
          {
...
public:
  static Quantizer *generate(Property &property)
  {
    ...
    fprintf(stderr, "%s::%d generate localIDByteSize %lu\n", __FILE__, __LINE__, localIDByteSize);
    ...
    else
    {
      if (localIDByteSize == 4)
      {
        ...
        fprintf(stderr, "%s::%d generate uint32 localIDByteSize %lu\n", __FILE__, __LINE__, localIDByteSize);
      }
      else if (localIDByteSize == 2)
      {
        quantizer = new QuantizerInstance<uint16_t>;
        fprintf(stderr, "%s::%d generate uint16 localIDByteSize %lu\n", __FILE__, __LINE__, localIDByteSize);
#ifdef NGTQ_QBG
      }
      else if (localIDByteSize == 1)
      {
        quantizer = new QuantizerInstance<uint8_t>;
        fprintf(stderr, "%s::%d generate uint8 localIDByteSize %lu\n", __FILE__, __LINE__, localIDByteSize);
#endif
      }
      else
      {
...
public:
                     ...
                     NGT::Property &localProperty)
  {
    ...
    try
    {
      ...
      CERR << "property dimension " << property.dimension << endl;
  ...
  {
    fprintf(stderr, "%s::%d create index quantizationCodebook_size(%lu), codebookIndex(%lu) objectIndex(%lu) %lu %lu\n",
            __FILE__, __LINE__, quantizationCodebook.size(), codebookIndex.size(), objectIndex.size(), beginID, endID);
