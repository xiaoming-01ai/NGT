...
namespace NGTQG {
  ...
  public:
    ...
    void construct(NGT::GraphRepository &graphRepository, NGTQ::Index &quantizedIndex, size_t maxNoOfEdges) {
      ...
      fprintf(stderr, "%s::%d QuantizedGraph construct %lu\n", __FILE__, __LINE__, graphRepository.size());
      for (size_t id = 1; id < graphRepository.size(); id++)
      {
        if ((graphRepository.size() > 100) && (id % ((graphRepository.size() - 1) / 100) == 0))
        {
          CERR <<  " "  << "# of processed objects=" << id << "/" << (graphRepository.size() - 1)
        ...
        for (auto i = node.begin(); i != node.end(); i++)
        {
          ...
          if ((*i).id == 0)
          {
            CERR <<  " "  << "something strange" << std::endl;
            abort();
            continue;
          }
  ...
  {
  public:
    Index(const std::string &indexPath, size_t maxNoOfEdges = 128, bool rdOnly = false) : NGT::Index(indexPath, rdOnly, false),
                                                                                          readOnly(rdOnly),
                                                                                          path(indexPath),
    ...
    {
      {
        ...
        else
        {
          if (readOnly)
          {
            CERR <<  " "  << "No quantized graph. Construct it temporarily." << std::endl;
          }
    ...
    static void buildQuantizedGraph(const std::string indexPath, size_t maxNumOfEdges = 128)
    {
      ...
      fprintf(stderr, "%s::%d realign %s\n", __FILE__, __LINE__, qgPath.c_str());
      ...
      CERR <<  " "  << "Quantized graph is completed." << std::endl;
    ...
    static void buildQuantizedObjects(const std::string quantizedIndexPath, NGT::ObjectSpace &objectSpace, bool insertion = false)
    {
      ...
      for (size_t id = 1; id < objectSpace.getRepository().size(); id++)
      {
        if (id % 100000 == 0)
        {
          CERR <<  " "  << "Processed " << id << " objects." << std::endl;
        }
        ...
        {
          CERR <<  " "  << "NGTQG::buildQuantizedObjects: Warning! Cannot get the object. " << id << std::endl;
          continue;
        }
    ...
    static void createQuantizedGraphFrame(const std::string quantizedIndexPath, size_t dimension, size_t pseudoDimension, size_t dimensionOfSubvector)
    {
      ...
      CERR << "dimension " << dimension << endl 
           << "pseudo dimension " << pseudoDimension << endl
           << "dimensionOfSubvector " <<  dimensionOfSubvector << endl;
