...
#define CERR std::cerr << __FILE__ << "(" << __FUNCTION__ << ")::" << __LINE__ << " "
...
#define	NGTThrowSpecificException(MESSAGE, TYPE)	throw NGT::TYPE(__FILE__, __FUNCTION__, (size_t)__LINE__, MESSAGE)
  ...
  class Args : public std::map<std::string, std::string>
  {
   public:
	...
	if (!status.second) {
	  CERR <<  " "  << "Args: Duplicated options. [" << opt << "]" << std::endl;
	}
      }
    }
  ...
  public:
	...
	if (*e != 0) {
	  CERR <<  " "  << "Common::extractVector: Warning! Not numerical value. [" << e << "] " << std::endl;
	  break;
	}
	object.push_back(v);
      }
  ...
  public:
    ...
    void begin()
    {
      ...
      // if (logFD < 0)
      // {
      //   CERR <<  " "  << "Logger: Cannot begin logging." << std::endl;
      //   logFD = -1;
      //   return;
      // }
      // savedFdNo = dup(fdNo);
  ...
  public:
    ...
    void extend(size_t idx)
    {
      if (idx >= allocatedSize)
      {
        ...
        if (size > 0xffff)
        {
          CERR <<  " "  << "CompactVector is too big. " << size << std::endl;
          abort();
        }
        reserve(size);
      }
  ...
  public:
    ...
    float getf(const std::string &key, float defvalue)
    {
      ...
      if (it != end())
      {
        ...
        if (*e != 0)
        {
          CERR <<  " "  << "Warning: Illegal property. " << key << ":" << it->second << " (" << e << ")" << std::endl;
          return defvalue;
        }
        return val;
      }
    ...
    void load(std::ifstream &is)
    {
      ...
      while (getline(is, line))
      {
        ...
        if (tokens.size() != 2)
        {
          CERR <<  " "  << "Property file is illegal. " << line << std::endl;
          continue;
        }
  ...
  namespace Serializer
  {
    ...
    void readAsText(std::istream &is, TYPE &v)
    {
      if (typeid(TYPE) == typeid(unsigned char))
      {
        ...
        if (tmp > 255)
        {
          CERR <<  " "  << "Error! Invalid. " << tmp << std::endl;
        }
        v = (TYPE)tmp;
      }
      else
    ...
    void readAsText(std::istream &is, TYPE *v, size_t s)
    {
      ...
      if (s != size)
      {
        CERR <<  " "  << "readAsText: something wrong. " << size << ":" << s << std::endl;
        return;
      }
      for (unsigned int i = 0; i < s; i++)
      {
  ...
  public:
    ...
    void extend(size_t idx, SharedMemoryAllocator &allocator)
    {
      if (idx >= allocatedSize)
      {
        ...
        if (size > 0xffffffff)
        {
          CERR <<  " "  << "Vector is too big. " << size << std::endl;
          abort();
        }
  ...
  public:
    ...
    {
      CERR <<  " "  << "DynamicLengthVector cannot be copied." << std::endl;
      abort();
    }
  ...
  public:
    ...
    void removedListPush(size_t id)
    {
      ...
      Vector<size_t>::iterator rmi = std::lower_bound(removedList->begin(allocator), removedList->end(allocator), id, std::greater<size_t>());
      if ((rmi != removedList->end(allocator)) && ((*rmi) == id))
      {
        CERR <<  " "  << "removedListPush: already existed! continue... ID=" << id << std::endl;
        return;
      }
    ...
    void deserializeAsText(std::ifstream &is, ObjectSpace *objectspace = 0)
    {
      ...
      for (size_t i = 0; i < s; i++)
      {
        ...
        if (i != idx)
        {
          CERR <<  " "  << "PersistentRepository: Error. index of a specified import file is invalid. " << idx << ":" << i << std::endl;
        }
        char type;
  ...
  public:
    ...
    void deserializeAsText(std::ifstream &is, ObjectSpace *objectspace = 0)
    {
      ...
      for (size_t i = 0; i < s; i++)
      {
        ...
        if (i != idx)
        {
          CERR <<  " "  << "Repository: Error. index of a specified import file is invalid. " << idx << ":" << i << std::endl;
        }
        char type;
        NGT::Serializer::readAsText(is, type);
        switch (type)
