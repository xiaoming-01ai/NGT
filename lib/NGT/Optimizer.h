  ...
  public:
	...
	static void checkAndGetSize(std::istream &resultStream, size_t &resultDataSize)
	{
						...
						if (line.size() != 0 && line.at(0) == '#')
						{
							...
							if (gtf.size() >= 1 && gtf[0] == "# End of Search")
							{
								...
								else
								{
									...
									{
										warn = true;
										CERR <<  " "  << "Warning!: Result sizes are inconsistent! $prevDataCount:$dataCount" << std::endl;
										CERR <<  " "  << "  Line No." << lineCount << ":" << lastDataLine << std::endl;
										if (prevDataCount < dataCount)
										{
											prevDataCount = dataCount;
										}
		...
		if (warn)
		{
			...
			CERR <<  " "  << " Check if the result number $$resultDataSize is correct." << std::endl;
			CERR <<  " "  << "Warning! ****************************************************************************" << std::endl;
		}
	}
		...
		while (getline(resultStream, line))
		{
			...
			if (line.size() != 0 && line.at(0) == '#')
			{
				...
				if (tf.size() >= 1 && tf[0] == "# Query No.")
				{
						...
						while (getline(resultStream, line))
						{
							...
							if (line.size() != 0 && line.at(0) == '#')
							{
								...
								else if (gtf.size() >= 1 && gtf[0] == "# End of Search")
								{
									...
									{
										CERR <<  " "  << "Warning! ****************************************************************************" << std::endl;
										CERR <<  " "  << "  Use $resultDataSize instead of $dataCount as the result size to compute accuracy. " << std::endl;
										CERR <<  " "  << "    # of the actual search resultant objects=" << dataCount << std::endl;
										CERR <<  " "  << "    the specified # of search objects or # of the ground truth data=" << resultDataSize << std::endl;
										CERR <<  " "  << "    Line No.=" << lineNo << " Query No.=" << queryNo << " Result No.=" << resultNo << std::endl;
										CERR <<  " "  << "Warning! ****************************************************************************" << std::endl;
									}
							...
							if (result.size() < 3)
							{
								CERR <<  " "  << "result format is wrong. " << std::endl;
								abort();
							}
							size_t rank = NGT::Common::strtol(result[0]);
							size_t id = NGT::Common::strtol(result[1]);
							...
							if (rank != dataCount)
							{
								CERR <<  " "  << "inner error! $rank $dataCount !!" << std::endl;
								;
								abort();
							}
						}
					...
					{
						CERR <<  " "  << "Fatal error! : Cannot find query No. " << queryNo << std::endl;
						abort();
					}
				}
			}
	...
	std::pair<size_t, double> adjustBaseSearchEdgeSize(std::stringstream &queries, Command::SearchParameters &searchParameters, std::stringstream &gtStream, std::pair<float, float> accuracyRange, float marginInit = 0.2, size_t prevBase = 0)
	{
		...
		while (true)
		{
			try
			{
				...
				CERR <<  " "  << "adjustBaseSearchEdgeSize: explore for the margin " << margin << ", " << baseStart << "..." << std::endl;
						...
						if (times.count(base) == 0)
						{
							for (;;)
							{
								...
								catch (NGT::Exception &err)
								{
									...
									{
										CERR <<  " "  << "Warning: Cannot adjust the base edge size." << err.what() << std::endl;
										CERR <<  " "  << "Try again with the next base" << std::endl;
										NGTThrowException("**Retry**");
									}
									if (margin > 0.4)
									{
										CERR <<  " "  << "Warning: Cannot adjust the base even for the widest margin " << margin << ". " << err.what();
										NGTThrowException("**Retry**");
									}
									else
									{
										CERR <<  " "  << "Warning: Cannot adjust the base edge size for margin " << margin << ". " << err.what() << std::endl;
										CERR <<  " "  << "Try again for the next margin." << std::endl;
										margin += 0.05;
									}
								}
							}
	...
	{
		CERR <<  " "  << "adjustBaseSearchEdgeSize: Extract queries for GT..." << std::endl;
		std::stringstream queries;
		extractQueries(querySize, queries);

		CERR <<  " "  << "adjustBaseSearchEdgeSize: create GT..." << std::endl;
	...
	std::pair<size_t, double> adjustRateSearchEdgeSize(std::stringstream &queries, Command::SearchParameters &searchParameters, std::stringstream &gtStream, std::pair<float, float> accuracyRange, float marginInit = 0.2, size_t prevRate = 0)
	{
		...
		while (true)
		{
			try
			{
				...
				size_t minRate = 0;
				std::map<size_t, double> times;
				CERR <<  " "  << "adjustRateSearchEdgeSize: explore for the margin " << margin << ", " << rateStart << "..." << std::endl;
							...
							for (;;)
							{
								...
								catch (NGT::Exception &err)
								{
									if (err.getMessage().find("Error!! Epsilon") != std::string::npos &&
										err.getMessage().find("is too large") != std::string::npos)
									{
										CERR <<  " "  << "Warning: Cannot adjust the rate of edge size." << err.what() << std::endl;
										CERR <<  " "  << "Try again with the next rate" << std::endl;
										NGTThrowException("**Retry**");
									}
									if (margin > 0.4)
									{
										CERR <<  " "  << "Error: Cannot adjust the rate even for the widest margin " << margin << ". " << err.what();
										NGTThrowException("**Retry**");
									}
									else
									{
										CERR <<  " "  << "Warning: Cannot adjust the rate of edge size for margin " << margin << ". " << err.what() << std::endl;
										CERR <<  " "  << "Try again for the next margin." << std::endl;
										margin += 0.05;
									}
								}
	...
	std::pair<size_t, size_t> adjustSearchEdgeSize(std::pair<float, float> baseAccuracyRange, std::pair<float, float> rateAccuracyRange, size_t querySize, double epsilon, float margin = 0.2)
	{
		...
		try
		{
			CERR <<  " "  << "adjustSearchEdgeSize: Extract queries for GT..." << std::endl;
			extractQueries(querySize, queries);
			CERR <<  " "  << "adjustSearchEdgeSize: create GT..." << std::endl;
			createGroundTruth(index, epsilon, searchParameters, queries, gtStream);
		}
		catch (NGT::Exception &err)
		{
			CERR <<  " "  << "adjustSearchEdgeSize: Error!! Cannot adjust. " << err.what() << std::endl;
			redirector.end();
		...
		for (;;)
		{
			try
			{
				...
				CERR <<  " "  << "adjustRateSearchEdgeSize: Base: base=" << prevBase.first << "->" << base.first << ",rate=" << prevRate.first << "->" << rate.first << std::endl;
				...
				CERR <<  " "  << "adjustRateSearchEdgeSize: Rate base=" << prevBase.first << "->" << base.first << ",rate=" << prevRate.first << "->" << rate.first << std::endl;
				...
				{
					CERR <<  " "  << "adjustRateSearchEdgeSize: Warning! Found an infinite loop." << std::endl;
			...
			{
				CERR <<  " "  << "adjustRateSearchEdgeSize: Error!! Cannot adjust. " << err.what() << std::endl;
				redirector.end();
				return std::pair<size_t, size_t>(0, 0);
			}
		}
	...
	static void adjustSearchEdgeSize(Args &args)
	{
		...
		{
			CERR <<  " "  << "ngt: Error: DB is not specified" << std::endl;
			CERR <<  " "  << usage << std::endl;
			return;
		}
		...
		CERR <<  " "  << "adjustRateSearchEdgeSize: # of queries=" << querySize << std::endl;
	...
	static void evaluate(Args &args)
	{
		...
		{
			CERR <<  " "  << "ground truth is not specified." << std::endl;
			CERR <<  " "  << usage << std::endl;
			return;
		}
		try
		...
		{
			CERR <<  " "  << "result file is not specified." << std::endl;
			CERR <<  " "  << usage << std::endl;
			return;
		}
		...
		if (resultSize != 0)
		{
			CERR <<  " "  << "The specified number of results=" << resultSize << std::endl;
		}
		...
		if (args.getChar("m", '-') == 'r')
		{
			CERR <<  " "  << "Recall" << std::endl;
			recall = true;
		}
		bool approximateDistance = false;
		if (args.getChar("m", '-') == 'a')
		{
			CERR <<  " "  << "Approximate distance" << std::endl;
		...
		if (!resultStream)
		{
			CERR <<  " "  << "Cannot open the specified target file. " << resultFile << std::endl;
			CERR <<  " "  << usage << std::endl;
			return;
		}
		...
		if (!gtStream)
		{
			CERR <<  " "  << "Cannot open the specified GT file. " << gtFile << std::endl;
			CERR <<  " "  << usage << std::endl;
			return;
		}
