...
void NGT::Index::createGraph(const string &database, NGT::Property &prop, const string &dataFile, size_t dataSize, bool redirect)
{
  if (prop.dimension == 0)
  {
    NGTThrowException("Index::createGraphAndTree. Dimension is not specified.");
...
void NGT::Index::loadAndCreateIndex(Index &index, const string &database, const string &dataFile, size_t threadSize, size_t dataSize)
{
  NGT::Timer timer;
  timer.start();
  if (dataFile.size() != 0)
...
void NGT::Index::Property::set(NGT::Property &prop)
{
  ...
  if (prop.threadPoolSize != -1)
    threadPoolSize = prop.threadPoolSize;
  if (prop.objectType != ObjectSpace::ObjectTypeNone)
    objectType = prop.objectType;
  if (prop.distanceType != DistanceType::DistanceTypeNone)
    distanceType = prop.distanceType;
  if (prop.indexType != IndexTypeNone)
    indexType = prop.indexType;
  if (prop.databaseType != DatabaseTypeNone)
    databaseType = prop.databaseType;
  if (prop.objectAlignment != ObjectAlignmentNone)
    objectAlignment = prop.objectAlignment;
  if (prop.pathAdjustmentInterval != -1)
    pathAdjustmentInterval = prop.pathAdjustmentInterval;
  if (prop.prefetchOffset != -1)
    prefetchOffset = prop.prefetchOffset;
  if (prop.prefetchSize != -1)
    prefetchSize = prop.prefetchSize;
  if (prop.accuracyTable != "")
...
void NGT::GraphIndex::loadIndex(const string &ifile, bool readOnly, bool graphDisabled)
{
...
void NGT::GraphIndex::saveProperty(const std::string &file)
{
  NGT::Property::save(*this, file);
}

void NGT::GraphIndex::exportProperty(const std::string &file)
{
...
void GraphIndex::createIndex(size_t threadPoolSize, size_t sizeOfRepository)
{
...
bool NGT::GraphIndex::showStatisticsOfGraph(NGT::GraphIndex &outGraph, char mode, size_t edgeSize)
{
  ...
  for (size_t id = 1; id < graph.size(); id++)
  {
    ...
    {
      CERR   << __FILE__ << "::" << __LINE__ << " "  << "ngt info: Error. Cannot get the node. ID=" << id << ":" << err.what() << std::endl;
      valid = false;
      continue;
    }
    numberOfNodes++;
    ...
    {
      CERR   << __FILE__ << "::" << __LINE__ << " "  << "Processed " << numberOfNodes << std::endl;
    }
    ...
    for (size_t i = 0; i < esize; i++)
    {
      ...
      if (n.id == 0)
      {
        CERR   << __FILE__ << "::" << __LINE__ << " "  << "ngt info: Warning. id is zero." << std::endl;
        valid = false;
      }
      indegreeCount[n.id]++;
  ...
  if (mode == 'a')
  {
    ...
    for (size_t id = 1; id < graph.size(); id++)
    {
      ...
      for (size_t i = 0; i < node.size(); i++)
      {
        ...
        {
          count++;
          CERR   << __FILE__ << "::" << __LINE__ << " "  << "Directed edge! " << id << "->" << node[i].id << " no object. " << node[i].id << std::endl;
          continue;
        }
        ...
        if (!found)
        {
          CERR   << __FILE__ << "::" << __LINE__ << " "  << "Directed edge! " << id << "->" << node[i].id << " no edge. " << node[i].id << "->" << id << std::endl;
          count++;
        }
      }
    }
    CERR   << __FILE__ << "::" << __LINE__ << " "  << "The number of directed edges=" << count << std::endl;
  }
  ...
  for (size_t id = 1; id < graph.size(); id++)
  {
    ...
    {
      CERR   << __FILE__ << "::" << __LINE__ << " "  << "ngt info: Warning. Cannot get the node. ID=" << id << ":" << err.what() << std::endl;
      continue;
    }
    NGT::GraphNode &node = *n;
  ...
  for (size_t id = 1; id < graph.size(); id++)
  {
    ...
    if (indegreeCount[id] == 0)
    {
      ...
      CERR   << __FILE__ << "::" << __LINE__ << " "  << "Error! The node without incoming edges. " << id << std::endl;
      valid = false;
    }
  ...
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The number of the objects:\t" << outGraph.getNumberOfObjects() << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The number of the indexed objects:\t" << outGraph.getNumberOfIndexedObjects() << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The size of the object repository (not the number of the objects):\t"
            << (repo.size() == 0 ? 0 : repo.size() - 1) << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The number of the removed objects:\t" << removedObjectCount << "/"
            << (repo.size() == 0 ? 0 : repo.size() - 1) << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The number of the nodes:\t" << numberOfNodes << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The number of the edges:\t" << numberOfOutdegree << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The mean of the edge lengths:\t" << std::setprecision(10)
            << (numberOfOutdegree != 0.0 ? distance / (double)numberOfOutdegree : 0) << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The mean of the number of the edges per node:\t"
            << (numberOfNodes != 0.0 ? (double)numberOfOutdegree / (double)numberOfNodes : 0) << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The number of the nodes without edges:\t" << numberOfNodesWithoutEdges << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The maximum of the outdegrees:\t" << maxNumberOfOutdegree << std::endl;
  if (minNumberOfOutdegree == SIZE_MAX)
  {
    CERR   << __FILE__ << "::" << __LINE__ << " "  << "The minimum of the outdegrees:\t-NA-" << std::endl;
  }
  else
  {
    CERR   << __FILE__ << "::" << __LINE__ << " "  << "The minimum of the outdegrees:\t" << minNumberOfOutdegree << std::endl;
  }
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The number of the nodes where indegree is 0:\t" << numberOfNodesWithoutIndegree << std::endl;
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "The maximum of the indegrees:\t" << maxNumberOfIndegree << std::endl;
  if (minNumberOfIndegree == INT64_MAX)
  {
    CERR   << __FILE__ << "::" << __LINE__ << " "  << "The minimum of the indegrees:\t-NA-" << std::endl;
  }
  else
  {
    CERR   << __FILE__ << "::" << __LINE__ << " "  << "The minimum of the indegrees:\t" << minNumberOfIndegree << std::endl;
  }
  CERR   << __FILE__ << "::" << __LINE__ << " "  << "#-nodes,#-edges,#-no-indegree,avg-edges,avg-dist,max-out,min-out,v-out,max-in,min-in,v-in,med-out,"
  ...
  if (mode == 'h')
  {
    CERR   << __FILE__ << "::" << __LINE__ << " "  << "#\tout\tin" << std::endl;
    for (size_t i = 0; i < outdegreeHistogram.size() || i < indegreeHistogram.size(); i++)
    {
      ...
      CERR   << __FILE__ << "::" << __LINE__ << " "  << i << "\t" << out << "\t" << in << std::endl;
    }
  }
  else if (mode == 'p')
  {
    CERR   << __FILE__ << "::" << __LINE__ << " "  << "ID\toutdegree\tindegree" << std::endl;
    for (size_t id = 1; id < graph.size(); id++)
    {
      CERR   << __FILE__ << "::" << __LINE__ << " "  << id << "\t" << outGraph.getNode(id)->size() << "\t" << indegreeCount[id] << std::endl;
    }
  }
  return valid;
}

void GraphAndTreeIndex::createIndex(size_t threadPoolSize, size_t sizeOfRepository)
{
...
{
  cerr <<  __FILE__ << "::" << __LINE__ << " " << "create index " << endl;
  Timer timer;
  size_t timerInterval = 100000;
  size_t timerCount = timerInterval;
  size_t count = 0;
